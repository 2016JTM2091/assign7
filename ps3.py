...
city={"ROORKEE":'111',"NEW DELHI":'110',"TARAKESWAR":'101',"KOLKATA":'100'} 
district={"IIT ROORKEE":'000',"NIT TRICHY":'001',"IIT DELHI":'010',"KALYANI":'011'}
 ...
 a=input("enter the operation you want to perform")
  if a==1:
   ...
   state.update({s:m}) c=raw_input("enter the city name you want to add") 
   ...
   if t1 in state.keys(): state.update({t1:t11}) 
   else : print 'not a valid state' 
   t2=raw_input("enter the district whose code you want to modify\n") 
   t21=raw_input("enter the new code\n") 
   if t2 in district.keys(): district.update({t2:t21}) 
   else : print 'not a valid district' 
   t3=raw_input("enter the city whose code you want to modify\n") 
   t31=raw_input("enter the new code\n") 
   if t3 in city.keys(): city.update({t3:t31}) 
   else : print 'not a valid city' 
   else : 
   ...
   if l1 in district.keys(): print district.get(l2, default=None) 
   if l1 in city.keys(): 
   print city.get(l3, default=None) 
   print "Human Readable code" 
   if l1 in state.keys(): 
   print if l1 in state.keys(): print state.get(l4, default=None)
